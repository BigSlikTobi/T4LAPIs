#!/usr/bin/env python3
"""
CLI script for loading Football Study Hall (FTN) data.
...
from src.core.data.loaders.ftn import FootballStudyHallDataLoader
from src.core.utils.cli import setup_cli_parser, setup_cli_logging, print_results, handle_cli_errors


@handle_cli_errors
def main():
    parser = setup_cli_parser("Load FTN data into the database")
    ...
    try:
        ...
        op = f"FTN data load for seasons {', '.join(map(str, args.years))}{week_suffix}{suffix}"
        print_results(result, op, args.dry_run)

        if result.get("success") and not args.dry_run:
            count = loader.get_record_count()
            print(f"Total FTN records in database: {count:,}")
